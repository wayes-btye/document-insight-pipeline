...
| **Temperature default** | 0.3 (configurable via `provider.temperature`) | 0.0 (fully deterministic); 1.0 (provider default); leave unset | Insight extraction is not safety-critical reasoning; it's pattern-matching and clustering, which benefits from non-zero sampling. Empirically: gpt-4o-mini at temp=0 gave worse `doc_coverage` (0.21) and `high_theme_recall` (0.50) than at temp=1.0 (0.43 / 1.0). 0.3 keeps most of the reproducibility benefit (consistency_jaccard typically 0.7+) without crippling creativity. Users who want fully reproducible runs (e.g. debugging the eval) can set 0.0; users who want richer phrasing can set 0.7+. |
